...
`randomness_report.md`, `extended_report.md` (KS test, entropy,
month/year/weekday homogeneity), `backtest_report.md`, and two PNG
charts.
...
- **§4** Extended methods — KS-test cards, Shannon entropy table,
  month/year/weekday homogeneity tabs, and a monthly trend line chart
- **§5** Backtest comparison — strategy table (including the golden-ratio
  strategy) + cumulative-bankroll line chart, tabs per target category
  (shows a "not enough data yet" message until `real_draws.csv` has grown
  past the warm-up threshold — currently needs >35 draws)
...
- **Kolmogorov-Smirnov goodness-of-fit** (a second, shape-based
  uniformity check alongside chi-square) on `two_digit`, `three_digit`,
  and `four_digit`
- **Shannon entropy ratio** (observed entropy vs. maximum possible) for
  `two_digit` and `three_digit` — close to 1.0 means close to maximally
  random
- **Homogeneity tests by month / year / weekday** — does a digit
  position's distribution shift across time groupings? A legitimate
  question for a physical/electronic draw mechanism, tested with
  chi-square tests of independence (contingency tables), separate from
  overall-uniformity testing
- A monthly mean trend line for `two_digit`, to visually show that
  apparent "patterns" over time are noise fluctuating around the
  theoretical expected value (49.5)
...
- `GoldenRatioSequence` — generates numbers from the golden-ratio
  equidistribution sequence (n·φ mod 1), a real low-discrepancy sampling
  technique. It's included specifically so its lack of edge can be
  demonstrated empirically rather than just asserted: the ratio's
  equidistribution property describes how a *deterministic* sequence
  spreads out over many steps, which has no mathematical bearing on what
  an *independent random draw* will produce next.
...
**Backtest sensitivity, 10-50 draws** (also `backtest.py`): the same
backtest re-run on just the last N draws, for N = 10, 15, 20, ..., 50 —
shown as an ROI-vs-window-size chart under §6 of the dashboard. At this
sample size, statistical power is very low, so ROI swings wildly between
window sizes (a single lucky hit can show as +200% ROI) — this is
included specifically to demonstrate *why* the main backtest requires
35+ draws before drawing conclusions, not to suggest these small-sample
results are meaningful on their own.

**k-value × payout-scenario sensitivity** (`backtest.py`, dashboard §7):
answers two questions together — "what if we bet more numbers per draw
(k)?" and "how much does the real payout table cost a player vs. a
theoretically fair one?"
- **Payout scenarios**: `real` uses the actual published multipliers
  confirmed from the live sanook.com page text (60× for 2-digit, 500× for
  3-digit, 6000× for 4-digit); `fair` uses the actuarially-neutral
  multiplier (exactly 1/probability: 100×/1000×/10000×) — i.e. what the
  payout would be with zero house edge. Comparing them gives the house
  edge directly: (1 − real/fair) → **40% for 2-digit, 50% for 3-digit**.
- **k values** tested: 1, 3, 5, 10, 20 numbers bet per draw. A random
  bettor's *expected* ROI is flat across k (each additional number has
  the same per-unit expected value) — what changes is variance: larger k
  means the realized hit rate converges faster to its true expected
  value, so results swing less. The dashboard shows this empirically
...
- The golden-ratio strategy is a real mathematical technique (low-
  discrepancy sampling) but has no theoretical basis for predicting
  independent random draws — it's included to *test* this claim
  empirically, not to endorse it. State this explicitly if you cite it,
  to avoid implying the toolkit treats it as a legitimate predictor.
- With many homogeneity tests run (month × year × weekday × 4 digit
  positions), some will show p < 0.05 by chance alone (multiple-testing
  effect) — treat isolated significant results in §4 the same way as
  isolated significant chi-square results in §1–2: not evidence on their
  own, only a consistently replicated pattern would be.

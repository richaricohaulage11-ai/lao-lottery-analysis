...
from .stats_analysis import digit_position_frequency, group_frequency
...
def plot_frequency_charts(df: pd.DataFrame, out_path: str) -> None:
    ...
    axes[0].set_title("Digit frequency — six_digit position 0 (leftmost)")
    axes[0].set_xlabel("Digit")
    axes[0].set_ylabel("Count")

    pos5 = digit_position_frequency(df, 5)
    axes[1].bar(pos5.index.astype(str), pos5.values, color="#55A868")
    axes[1].axhline(pos5.values.mean(), color="black", linestyle="--", linewidth=1)
    axes[1].set_title("Digit frequency — six_digit position 5 (rightmost)")
    axes[1].set_xlabel("Digit")
    axes[1].set_ylabel("Count")

    tb = group_frequency(df, "two_digit", 2)
    axes[2].bar(range(100), tb.values, color="#C44E52", width=1.0)
    axes[2].axhline(tb.values.mean(), color="black", linestyle="--", linewidth=1)
    axes[2].set_title("two_digit frequency (00-99, last 2 digits)")

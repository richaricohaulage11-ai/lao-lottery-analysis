...
stats_analysis.py
------------------
Randomness tests for a single 6-digit-per-draw lottery, where every
reported sub-prize (5/4/3/2 ตัว) is a trailing slice of the same 6-digit
number (see data_loader.py docstring).

Two families of test are computed, and they answer different questions:

1. Per-digit-position marginal tests (d0..d5): is each digit position, on
   its own, uniformly distributed 0-9?
2. Joint / interaction tests on two_digit, three_digit, and four_digit:
   even if every digit position is individually uniform, a GROUP of them
   could still be non-uniform if the positions are correlated with each
   other. Testing the joint frequency catches that — legitimate extra
   information, not double-counting.
"""
...
def digit_position_frequency(df: pd.DataFrame, position: int) -> pd.Series:
    """Frequency table (0-9) for digit position `position` (0=leftmost) of six_digit."""
    col = f"d{position}"
...
def chi_square_digit_position(df: pd.DataFrame, position: int, alpha: float = 0.05) -> ChiSquareResult:
    ...
    return _chi_square_from_counts(f"six_digit position {position} (marginal)", counts, alpha)
...
def full_randomness_report(df: pd.DataFrame, alpha: float = 0.05) -> str:
    ...
    lines.append(
        ...
        "slice of the SAME 6-digit draw (six_digit), not a separate random "
        "draw. See data_loader.py for exactly which digits map to which "
        "category."
    )
    lines.append("")

    lines.append("## Part 1: Marginal chi-square, per digit position of six_digit")
    sig_marginal = 0
    for pos in range(6):
    ...
    total_tests = 6 + len(joint_results)

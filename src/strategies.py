...
work on any of the real, purchasable prize categories: two_digit_bottom
(2 ตัวล่าง), two_digit_top (2 ตัวบน), or three_digit_top (3 ตัวบน). All are
slices of the same 4-digit draw (see data_loader.py) — the strategies
themselves don't need to know that, they just operate on whichever column
they're pointed at.
...
# Number of digits (and therefore zero-padding width / universe size) for
# each supported target column.
TARGET_DIGITS = {
    "two_digit_bottom": 2,
    "two_digit_top": 2,
    "three_digit_top": 3,
...
class Strategy:
    ...
    def __init__(self, target_column: str = "two_digit_bottom", k: int = 5):
...
class RandomBaseline(Strategy):
    ...
    def __init__(self, target_column: str = "two_digit_bottom", k: int = 5, seed: int | None = None):
...
class HotNumbers(Strategy):
    ...
    def __init__(self, target_column: str = "two_digit_bottom", k: int = 5, window: int = 50):
        super().__init__(target_column, k)
        self.window = window
    ...
    def predict(self, history: pd.DataFrame) -> set[str]:
        ...
        most_common = [num for num, _ in counts.most_common(self.k)]
...
class LastDrawRepeat(Strategy):
    ...
    def __init__(self, target_column: str = "two_digit_bottom", k: int = 5, lookback: int = 5):
        super().__init__(target_column, k)
